...
class XTTSSynthesizer(BaseSynthesizer):
    ...
    async def _generate(self, text):
        try:
            yield await self.__generate_http(text)
        except Exception as e:
            logger.error(f"Error in xtts generate: {e}")

    async def sender(self, text, end_of_llm_stream):
        logger.info(f"Sending to server: text={text}, end_of_llm_stream={end_of_llm_stream}")
        input_message = {
            "text": text,
            "model": "xtts",
            "language": self.language,
        ...
        logger.info(f"Sent to server: {input_message}")
            ...
            try:
                if self.websocket_connection is not None:
                    ...
                    if int(self.sampling_rate) != 24000:
                        logger.info(f"Changing sampling rate to {int(self.sampling_rate)}")
